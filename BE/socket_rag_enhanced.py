...
try:
    ...
    with open("enhanced_graph.png", "wb") as f:
        f.write(png_bytes)
    print("Enhanced graph image saved to 'enhanced_graph.png'")
except Exception as e:
    print(f"Failed to save enhanced graph image: {e}")

# Enhanced async function to handle user input
async def handle_user_input_enhanced(user_input):
    """Enhanced user input handler with better classification."""
    logger.info("Starting enhanced chatbot")
    if user_input.lower() in ['exit', 'quit']:
        logger.info("Exiting enhanced chatbot")
        return "Goodbye!"
...
if __name__ == "__main__":
    logger.info("Starting enhanced main execution")
    try:
        if platform.system() == "Emscripten":
            logger.info("Running in Emscripten environment")
            asyncio.ensure_future(test_enhanced_classification())
        else:
